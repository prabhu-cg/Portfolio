...
export function Timeline({ items, className, highlightFirst = false }: TimelineProps) {
  return (
    <motion.div
      initial="hidden"
      whileInView="visible"
          ...
          <motion.div key={item.title} variants={fadeUp} className="relative flex flex-1 gap-4 md:flex-col md:gap-4">
            <div className="flex flex-col items-center md:w-full">
              <div className="flex items-center md:w-full">
                <span
                  className={cn(
                    "flex size-9 shrink-0 items-center justify-center rounded-full border font-plex text-sm font-medium",
                    isHighlighted
                      ? "border-accent-strong bg-accent-light text-accent-strong"
            ...
            <div className="flex flex-col gap-1.5 pb-6 md:pb-0 md:pt-2">
              <h3
                className={cn(
                  ...
                  isHighlighted ? "text-accent-strong" : "text-ink"
                )}
              >
                {item.title}
              </h3>
